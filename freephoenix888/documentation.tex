  ...
  \item \textbf{Связи-типы:}
        \begin{itemize}
          ...
          \item Если нужно что бы экземпляр связи мог быть связан с любой
                связью,
                то нужно использовать встроенный тип \hyperlink{Core.Any.Def}{Any}
        \end{itemize}

  \item \textbf{Пакеты:}
        \begin{itemize}
          \item Существует встроенный в Deep тип
                \hyperlink{Core.Contain.Def}{Contain}, обозначающий
...
\begin{itemize}
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь типа
        \hyperlink{Core.Type.Def}{Type} \hyperlink{FAQ.HowToInsertLinkWithoutFromAndTo}{без
  ...
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь типа
        \hyperlink{Core.Type.Def}{Type} от Message до Conversation и
  ...
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь типа
        \hyperlink{Core.Type.Def}{Conversation}
        \hyperlink{FAQ.HowToInsertLinkWithoutFromAndTo}{без начала и конца}
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь типа
        \hyperlink{Core.Type.Def}{UrgentMessage}
        \hyperlink{FAQ.HowToInsertLinkWithoutFromAndTo}{без начала и конца}
...
\begin{itemize}
  ...
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь типа
        \hyperlink{Core.Type.Def}{Type} \hyperlink{FAQ.HowToInsertLinkWithFromAndTo}{с
...
\hyperlink{Core.Contain.Def}{Contain}. То есть что бы ваша связь находилась в
...
Создаём связь \texttt{Email} (тип: \texttt{\hyperlink{Core.Type.Def}{Type}})

Создаём \hyperlink{Core.SyncTextFile.Def}{SyncTextFile} содержащий такой код:
...
\hyperlink{Core.dockerSupportsJs.Def}{dockerSupportsJs}, до: наш
\hyperlink{Core.SyncTextFile.Def}{SyncTextFile})
...
\begin{itemize}
  \item Создаём связь \texttt{Email} (тип: \texttt{\hyperlink{Core.Type.Def}{Type}})
  \item Создаём \hyperlink{Core.SyncTextFile.Def}{SyncTextFile} содержащий такой
        код:
        \begin{lstlisting}
...
\begin{itemize}
  \item Создать \hyperlink{Core.SyncTextFile.Def}{SyncTextFile} с таким кодом:
        \begin{lstlisting}
(request, response) => {
    response.send("ok");
}
\end{lstlisting}
  \item Создать \hyperlink{Core.Handler.Def}{Handler} от
        \hyperlink{Core.dockerSupportsJs.Def}{dockerSupportsJs}
        до нашего \hyperlink{Core.SyncTextFile.Def}{SyncTextFile}
  \item Создать \hyperlink{Core.Route.Def}{Route}
  \item Создать \hyperlink{Core.HandleRoute.Def}{HandleRoute} от нашего
        \hyperlink{Core.Route.Def}{Route} до нашего \hyperlink{Core.Handler.Def}{Handler}
  \item Создать \hyperlink{Core.Router.Def}{Router}
  \item Создать \hyperlink{Core.RouterStringUse.Def}{RouterStringUse} от нашего
        \hyperlink{Core.Route.Def}{Route} до нашего \hyperlink{Core.Router.Def}{Router},
        с
        значением
        веб-маршрута, который должен обрабатываться, например /my-path
  \item Создать \hyperlink{Core.Port.Def}{Port} с значением порта, который должен
        обрабатываться, например
        5000
  \item Создать \hyperlink{Core.RouterListening.Def}{RouterListening} от нашего
        \hyperlink{Core.Router.Def}{Router} до нашего \hyperlink{Core.Port.Def}{Port}
\end{itemize}
...
связи, например \hyperlink{Core.Type.Def}{Type} и ниже найдите связь с таким
названием. Вы можете
заметить, что там есть как минимум два \hyperlink{Core.Type.Def}{Type}, один со
значком звёздочки, а
другой со значком папки. Связь с значком папки это
\hyperlink{Core.Contain.Def}{Contain}, следовательно нажав
на стрелку влево около этой связи, мы сможем увидеть откуда ведёт
\hyperlink{Core.Contain.Def}{Contain}
...
\hyperlink{Core.Contain.Def}{Contain}, следовательно
\hyperlink{Core.Contain.Def}{Contain} будет указывать на ту самую связь со
звёздочкой, то есть вы можете
выбрать \hyperlink{Core.Type.Def}{Type} либо нажав на стрелку вправо у его
\hyperlink{Core.Contain.Def}{Contain}'a, либо нажав на сам
\hyperlink{Core.Type.Def}{Type} (на название). Удостоверьтесь, что над полем для
...
\subsubsection{Связи}
\paragraph*{Type}
Тип Type\hypertarget{Core.Type.Def}{} используется для создания новых типов
\paragraph*{Contain}
Тип Contain\hypertarget{Core.Contain.Def}{} используется для обозначения
принадлежности, а так же для присваивания имени связи. Например пакет, что бы
содержать в себе другие связи - имеет \hyperlink{Core.Contain.Def}{Contain} от себя
до другой связи. Если вы хотите дать имя связи - обновите значение экземпляра
\hyperlink{Core.Contain.Def}{Contain}
\paragraph*{SyncTextFile}
Тип SyncTextFile\hypertarget{Core.SyncTextFile.Def}{} используется для хранения
любого строкового значения
\paragraph*{Any}
Тип Any\hypertarget{Core.Any.Def}{} может использоваться для ограничений
...
\paragraph*{Handler}
Тип Handler\hypertarget{Core.Handler.Def}{} используется для создания обработчика.
Его началом является экземпляр \hyperlink{supports.Def}{Supports}, а концом
любая связь, содержащая код в строковом значении
\paragraph*{Supports}
Тип Supports\hypertarget{Core.Handler.Def}{} используется для создания среды
выполнения обработчика. Примером среды выполнения обработчика является
\hyperlink{Core.dockerSupportsJs.Def}{dockerSupportsJs}
\paragraph*{dockerSupportsJs}
Связь dockerSupportsJs\hypertarget{Core.dockerSupportsJs.Def}{} используется для
поддержки JavaScript в Docker. Что бы использовать эту среду для обработчика,
нужно сделать эту связь началом связи \hyperlink{Core.Handler.Def}{Handler}
\paragraph*{HandleInsert}
Связь HandleInsert\hypertarget{Core.HandleInsert.Def}{} используется для обработки
вставки экземпляров определённого типа. Например, что бы обрабатывать вставку
экземпляров типа \hyperlink{Core.Type.Def}{Type}, нужно создать HandleInsert с
началом типом \hyperlink{Core.Type.Def}{Type} и концом обработчиком, то есть
экземпляром \hyperlink{Core.Handler.Def}{Handler}
\paragraph*{HandleUpdate}
Связь HandleUpdate\hypertarget{Core.HandleUpdate.Def}{} используется для обработки
обновлений экземпляров определённого типа. Например, что бы обрабатывать
обновления экземпляров типа \hyperlink{Core.Type.Def}{Type}, нужно создать
HandleUpdate с началом типом \hyperlink{Core.Type.Def}{Type} и концом обработчиком,
то есть экземпляром \hyperlink{Core.Handler.Def}{Handler}
\paragraph*{HandleUpdate}
Связь HandleDelete\hypertarget{Core.HandleDelete.Def}{} используется для обработки
удалений экземпляров определённого типа. Например, что бы обрабатывать удаления
экземпляров типа \hyperlink{Core.Type.Def}{Type}, нужно создать HandleDelete с
началом типом \hyperlink{Core.Type.Def}{Type} и концом обработчиком, то есть
экземпляром \hyperlink{Core.Handler.Def}{Handler}
\paragraph*{Tree}
Тип Tree\hypertarget{Core.Tree.Def}{} используется для создания дерева
\paragraph*{TreeIncludeDown}
Тип TreeIncludeDown\hypertarget{Core.TreeIncludeDown.Def}{} используется для
включения в дерево связи, по направлению вниз
\paragraph*{TreeIncludeUp}
Тип TreeIncludeUp\hypertarget{Core.TreeIncludeUp.Def}{} используется для включения в
дерево связи, по направлению вверх
\paragraph*{TreeIncludeNode}
Тип TreeIncludeNode\hypertarget{Core.TreeIncludeNode.Def}{} используется для
включения в дерево связи, у которой нет начала и конца
\paragraph*{Route}
Тип Route\hypertarget{Core.Route.Def}{} используется для создания веб-маршрута
\paragraph*{HandleRoute}
Связь HandleRoute\hypertarget{Core.HandleRoute.Def}{} используется для обработки
веб-маршрута

\paragraph*{Router}
Тип Router\hypertarget{Core.Router.Def}{} используется для создания роутера
\paragraph*{RouterStringUse}
Связь RouterStringUse\hypertarget{Core.RouterStringUse.Def}{} используется для
указания веб-маршрута, который должен обрабатываться
\paragraph*{Port}
Тип Port\hypertarget{Core.Port.Def}{} используется для создания порта
\paragraph*{RouterListening}
Связь RouterListening\hypertarget{Core.RouterListening.Def}{} используется для
указания порта, который должен обрабатываться
\paragraph*{Space}
Тип Space\hypertarget{Core.Space.Def}{} используется для создания пространства
...
\begin{itemize}
  ...
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь
        \hyperlink{Core.TreeIncludeNode.Def}{TreeIncludeNode} от
        экземпляра \hyperlink{Core.Tree.Def}{Tree} до MyNode1
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь
        \hyperlink{Core.TreeIncludeNode.Def}{TreeIncludeNode} от
        экземпляра \hyperlink{Core.Tree.Def}{Tree} до MyNode2
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь
        \hyperlink{Core.TreeIncludeDown.Def}{TreeIncludeDown} от
        экземпляра \hyperlink{Core.Tree.Def}{Tree} до MyDownLink
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь
        \hyperlink{Core.TreeIncludeNode.Def}{TreeIncludeUp} от
  ...
  \item \hyperlink{DeepCase.InsertLink.Def}{Вставьте} связь типа
        \hyperlink{Core.Space.Def}{Space}, что бы пеерстать видеть наши связи и показать
        работу деревьев, используя Traveller
  \item Открываем контекстное меню экземпляра \hyperlink{Core.Space.Def}{Space} и
...
\subparagraph{Вставка связи с типом \hyperlink{Core.Type.Def}{Type}}
\begin{lstlisting}
await deep.insert({
...
\subparagraph{Получение связи с типом \hyperlink{Core.Type.Def}{Type}}
\begin{lstlisting}
await deep.select({
  type_id: await deep.id("@deep-foundation/core", "Type"),
});
...
Удаление связи с типом \hyperlink{Core.Type.Def}{Type}
...
значение \hyperlink{Core.Contain.Def}{Contain} связи, которая указывает на вашу
связь. \\
\textbf{Исключения:}
\begin{itemize}
  \item \hyperlink{Core.Package.Def}{Package}
  \item \hyperlink{Core.Package.Def}{User}
\end{itemize}
